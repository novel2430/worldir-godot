...
func _test_object_patch_and_animation() -> void:
    ...
    var stable := active.get_node("Entities/stable_crate") as Node3D
    var mover := active.get_node("Entities/moving_crate") as Node3D
    ...
    var old_road := active.get_node("Networks/main_path") as Node3D
    ...
    assert((active.get_node("Entities/stable_crate") as Node3D).get_instance_id() == stable_id)
    assert((active.get_node("Entities/moving_crate") as Node3D).get_instance_id() == mover_id)
    assert((active.get_node("Distributions/trees/trees_000") as Node3D).get_instance_id() == stable_tree_id)
    assert((active.get_node("Entities/moving_crate") as Node3D).transform.is_equal_approx(
        (new_world.find_entity("moving_crate") as ResolvedEntity).transform
    ))
    ...
    var new_road := active.get_node("Networks/main_path") as Node3D
...
func _test_terrain_swap_preserves_unchanged_nodes() -> void:
    ...
    var active := world_root.get_node("GeneratedWorld")
    var entity := active.get_node("Entities/stable_crate") as Node3D
    ...
    active = world_root.get_node("GeneratedWorld")
    assert(active.get_instance_id() == active_id)
    assert((active.get_node("Entities/stable_crate") as Node3D).get_instance_id() == entity_id)
...
func _object_world(updated: bool) -> ResolvedWorld:
    ...
    world.entities.append(_entity("stable_crate", Vector3(-12.0, 0.0, 0.0)))
    ...
    road.id = "main_path"
    road.semantic_type = "path"
    road.surface_kind = "path"
    road.width = 5.5
...
func _terrain_world(height: float) -> ResolvedWorld:
    ...
    world.entities.append(_entity("stable_crate", Vector3.ZERO))
...
func _entity(object_id: String, position: Vector3) -> ResolvedEntity:
    ...
    entity.id = object_id
    entity.semantic_type = "crate"
    entity.prototype_id = "oweng_crate_01"
